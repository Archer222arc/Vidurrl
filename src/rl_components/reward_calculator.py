...
class RewardCalculator:
    ...
    def calculate_reward(
        ...
        if self.mode == "delta":
            ...
            # Apply smooth scaling to preserve gradient information
            # Use tanh with scale factor to map [-∞,∞] → [-1,1] smoothly
            # This preserves reward differences while avoiding numerical instability
            reward_scale = 3.0  # Controls sensitivity: larger = more gradual scaling
            reward = float(np.tanh(raw_reward / reward_scale))
            ...
            # Apply smooth scaling consistent with delta mode
            reward_scale = 3.0  # Same scale as delta mode for consistency
            reward = float(np.tanh(raw_reward / reward_scale))
            ...
            # Legacy load balance penalties
            load_balance_penalty = self.calculate_load_balance_penalty(replica_ids, get_replica_scheduler_fn)

            # Combine components with new weighting structure
            raw_reward = (
            ...
            # Apply tanh scaling for bounded output
