...
class ChunkTrainer:
    ...
    def __init__(self, config: Dict):
        ...
        self.training_config_script = self.repo_root / "src/core/utils/infrastructure/config/training_config.py"
    ...
    def _execute_chunk(self, chunk_id: int, requests: int, is_first_chunk: bool, resume_checkpoint: str) -> bool:
        ...
        try:
            ...
            self.progress_manager.update_chunk_progress(chunk_id, "failed", "", requests)
            # 根据CLAUDE.md要求：禁止fallback模式，直接抛出错误
            raise RuntimeError(f"Chunk {chunk_id} 训练失败: {e}") from e
