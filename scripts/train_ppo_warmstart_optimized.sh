...
if [[ "$FORCE_WARMSTART" == "true" ]]; then
    ...
    # 外部预训练模型模式：优先使用外部模型，不询问checkpoint恢复
    echo "🌟 外部预训练模型模式已启用"
    if [ -f "${LATEST_CHECKPOINT}" ]; then
        echo "⚠️  检测到existing checkpoint: ${LATEST_CHECKPOINT}"
        echo "📊 Checkpoint信息:"
        ...
        echo ""
        echo "🤔 发现conflicting选项："
        echo "   - 外部预训练模型: ${EXTERNAL_PRETRAIN}"
        echo "   - Existing checkpoint: ${LATEST_CHECKPOINT}"
        echo ""
        echo "请选择优先级:"
        echo "   1) 使用外部预训练模型 (推荐 - 跳过warmstart，直接PPO训练)"
        echo "   2) 使用外部预训练模型执行完整warmstart (包含BC微调)"
        echo "   3) 恢复existing checkpoint (忽略外部预训练模型)"
        echo "   q) 退出脚本"
        ...
        while true; do
            ...
            case $choice in
                1 )
                    echo "✅ 选择使用外部预训练模型，跳过warmstart直接进行PPO训练"
                    SKIP_WARMSTART=true
                    # 外部预训练模型作为PPO的初始actor
                    break;;
                2 )
                    echo "✅ 选择使用外部预训练模型，执行完整warmstart训练"
                    SKIP_WARMSTART=false
                    break;;
                3 )
                    echo "✅ 选择恢复existing checkpoint，忽略外部预训练模型"
                    RESUME_ARGS="--p_p_o_global_scheduler_modular_config_load_checkpoint ${LATEST_CHECKPOINT}"
                    SKIP_WARMSTART=true
                    echo "⚠️  外部预训练模型 ${EXTERNAL_PRETRAIN} 将被忽略"
                    break;;
                [Qq]* )
                    echo "👋 退出脚本"
                    exit 0;;
        ...
        echo "🆕 未发现checkpoint，检测到外部预训练模型"
        echo "🤔 如何使用外部预训练模型？"
        echo "   1) 跳过warmstart，直接PPO训练 (推荐 - 外部模型已训练)"
        ...
        while true; do
            ...
            case $choice in
                1 )
                    echo "✅ 选择跳过warmstart，使用外部预训练模型直接进行PPO训练"
                    SKIP_WARMSTART=true
                    break;;
                2 )
                    echo "✅ 选择执行完整warmstart，对外部预训练模型进行BC微调"
                    SKIP_WARMSTART=false
                    break;;
                [Qq]* )
                    echo "👋 退出脚本"
...
if [ "$SKIP_WARMSTART" = true ]; then
    if [[ -n "$RESUME_ARGS" ]]; then
        ...
        if [[ -n "$EXTERNAL_PRETRAIN" ]]; then
            echo "   - ⚠️  外部预训练模型被忽略: ${EXTERNAL_PRETRAIN}"
        fi
    elif [[ -n "$EXTERNAL_PRETRAIN" ]]; then
        echo "   - 🚀 外部预训练直接模式: 跳过warmstart，直接PPO训练"
        echo "   - 预训练模型: ${EXTERNAL_PRETRAIN}"
        echo "   - 跳过: 示教数据收集 + BC预训练 (外部模型已训练)"
    fi
elif [[ -n "$EXTERNAL_PRETRAIN" ]]; then
    echo "   - 🌟 外部预训练warmstart模式: 使用高质量预训练模型"
    echo "   - 预训练模型: ${EXTERNAL_PRETRAIN}"
...
if [ "$CHUNK_MODE" = true ]; then
    ...
    echo "   - 预计块数: $(( (TOTAL_REQUESTS + CHUNK_SIZE - 1) / CHUNK_SIZE ))"
else
...
if [ "$CHUNK_MODE" = true ]; then
    ...
    python3 -m src.core.algorithms.training.chunk_trainer \
        --output-dir "${OUTPUT_DIR}" \
        --total-requests "${TOTAL_REQUESTS}" \
        --chunk-size "${CHUNK_SIZE}" \
        --config-file "${CONFIG_FILE:-configs/ppo_warmstart.json}" \
        --num-replicas "${NUM_REPLICAS}" \
        --qps "${QPS}" \
        --skip-warmstart "${SKIP_WARMSTART}" \
        --external-pretrain "${EXTERNAL_PRETRAIN}" \
        --pretrained-actor-path "${PRETRAINED_ACTOR_PATH}"

    CHUNK_EXIT_CODE=$?

    if [ $CHUNK_EXIT_CODE -eq 0 ]; then
        echo "✅ 分块训练完成"
        echo "📈 查看训练状态: bash scripts/show_training_status.sh"
    else
        echo "❌ 分块训练失败"
    fi
...
if [ "$SKIP_WARMSTART" = true ]; then
    if [[ -n "$RESUME_ARGS" ]]; then
        ...
        echo "🚀 [外部预训练] PPO训练 (使用外部预训练模型: ${EXTERNAL_PRETRAIN})..."
    fi
else
...
if [[ -n "$CONFIG_FILE" && -f "$CONFIG_FILE" ]]; then
    ...
    PPO_ARGS=$(python src/core/utils/infrastructure/config/training_config.py "configs/ppo_warmstart.json" "$OUTPUT_DIR")
fi
...
WARMSTART_ARGS=""
if [ "$SKIP_WARMSTART" = false ]; then
    # 执行warmstart: 使用warmstart模式和预训练模型
    ...
    # 跳过warmstart但使用外部预训练模型：直接加载外部预训练模型而不启用warmstart模式
...
if [ "$VERBOSE" = true ]; then
    ...
    python -m vidur.main \
      ...
      ${RESUME_ARGS} \
      ${WARMSTART_ARGS} \
      2>&1 | tee "${OUTPUT_DIR}/ppo_training.log"
else
    echo "🤫 静默模式: 输出仅保存到日志文件 ${OUTPUT_DIR}/ppo_training.log"
    python -m vidur.main \
      $PPO_ARGS \
      ${RESUME_ARGS} \
      ${WARMSTART_ARGS} \
